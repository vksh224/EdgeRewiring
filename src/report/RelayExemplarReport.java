 ...
 * the warmup period.
 */
public class RelayExemplarReport extends Report implements UpdateListener {
...
//				getProperty("Energy.value");
			
			@SuppressWarnings("unchecked")
			ArrayList<DTNHost> relayExemplarList = (ArrayList<DTNHost>)h.getComBus().getProperty(h.toString());
			//ArrayList<DTNHost> relayExemplarList = h.getRelayExemplarMap();
			//String relayExemplarString = h.getExemplarString();
//			if (value == null) {
//				throw new SimError("Host " + h + 
//						" is not using an energy aware router");
//			}			
			write(getSimTime()+"   "+h.toString() + " " +" "+h.getContactFrequency()
					+" "+ relayExemplarList+" "+h.getConnections().toString());
		}
